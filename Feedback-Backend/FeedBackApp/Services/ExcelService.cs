    ...
    public class ExcelService : IExcelService
    {
        ...
        public async Task<byte[]> ExportExcelAsync(int surveyId, int userId, string role)
        {
            ...
            _logger.LogInformation(
                "[EXCEL] Generating for Survey={SurveyId} Responses={Count} Questions={QCount}",
                surveyId, responses.Count, questions.Count);

            using var wb = new XLWorkbook();
            BuildResponsesSheet(wb, survey, questions, responses);
            BuildSummarySheet(wb, survey, questions, responses);
